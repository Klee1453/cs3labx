...
module Branch_Prediction(
    ...
    input load_use_hazzard,     /* 是否是 load, b-type紧邻的情况 */
...
//  reg [1:0]prev_predictBit;   // 上一周期的predictBit
    ...
    reg [63:0]PC_IF_buffer;      // PC_IF缓冲区, 用于生成上一周期PC_IF值
    reg [63:0]prev_PC_IF;        // 上一周期PC_IF_hash值
    reg prev_refetch;           // 缓冲refetch信号
    wire [4:0]prev_PC_hash = prev_PC_IF[6:2];
    
    reg [63:0]prev_PC_to_take;

    reg prev_load_use_hazzard, curr_load_use_hazzard;
...
//          prev_predictBit <= 2'b0;
            ...
            prev_taken <= 1'b0;
            prev_refetch <= 1'b0;
            prev_PC_to_take <= 64'b0;
            prev_load_use_hazzard <= 1'b0;
            curr_load_use_hazzard <= 1'b0;
        end
        else begin
            prev_load_use_hazzard <= curr_load_use_hazzard; // 将上一周期的load_use_hazzard存储
            curr_load_use_hazzard <= load_use_hazzard;      // 更新这一周期的load_use_hazzard
            if (~prev_load_use_hazzard) begin
...
//            predictBit_buffer <= curr_predictBit;
    ...
    assign PC_to_take = isJB_IF ? (hashCollision ? 1'b0 : BTB_predictedPC[PC_IF_hash]) : 64'b0;    
